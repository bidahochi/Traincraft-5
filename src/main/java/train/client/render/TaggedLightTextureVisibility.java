...
final class TaggedLightTextureVisibility {
    ...
    static boolean isVisible(
        ...
        for (DetectedLightFace face : surface.faces) {
            if (hasUsablePolygon(face) == false) continue;
            usablePolygonFound = true;
            if (polygonVisible(width, height, alpha, face)) return true;
        }
    ...
    private static boolean polygonVisible(
        ...
        for (int y = minimumY; y <= maximumY; y++) {
            ...
            for (int x = minimumX; x <= maximumX; x++) {
                if ((alpha[y * width + x] & 255) < CUTOUT_ALPHA_THRESHOLD) continue;
                float u = (x + 0.5F) / width;
                if (insidePolygon(u, v, face.textureU, face.textureV)) return true;
            }
        }
        return false;
    }
    ...
    private static boolean insidePolygon(float u, float v, float[] polygonU, float[] polygonV) {
        ...
        for (int current = 0; current < polygonU.length; current++) {
            ...
            if (onSegment(u, v, previousU, previousV, currentU, currentV)) return true;
    ...
    private static boolean onSegment(
        ...
        if (Math.abs(cross) > 1.0E-6F) return false;
    ...
    private static int lowerPixel(float normalized, int size) {
        ...
        if (normalized > 0.0F && wrapped == 0.0F) wrapped = 1.0F;
