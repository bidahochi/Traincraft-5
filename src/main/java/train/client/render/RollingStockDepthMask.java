...
/** Exact screen-space coverage of rolling-stock depth written during this frame. */
final class RollingStockDepthMask {
    ...
    static void beginModel(int ownerId) {
    ...
    private static void releaseTargets() {
        if (beforeDepthTexture != 0) GL11.glDeleteTextures(beforeDepthTexture);
        if (afterDepthTexture != 0) GL11.glDeleteTextures(afterDepthTexture);
        if (maskTexture != 0) GL11.glDeleteTextures(maskTexture);
