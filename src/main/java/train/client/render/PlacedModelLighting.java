...
/**
 * Java 8/fixed-function adapter for placed-model light extraction. Texture classification is
 * cached; animated part poses are captured per draw.
 */
public final class PlacedModelLighting {
    ...
    public static void begin(
        ...
        phases.add(phase0);
        if (phase1 != null) phases.add(phase1);
        PlacedModelLightProfile profile =
                new PlacedModelLightProfile(
                        "warning", off, phases, 1, 0xFF2020, 0.75F, 0.35F, 0.85F, true);
    ...
    public static void end() {
        ...
        try {
            if (context != null) submitClusters(context);
        } finally {
            ACTIVE.remove();
        }
    }
    ...
    public static PartLight beginPart(final ModelRendererTurbo part, float scale) {
        ...
        if (staticPart == null) return PartLight.NONE;
        ...
        if (authoredPhase == PlacedLightTexturePhases.NONE
                || authoredPhase != PlacedLightTexturePhases.STEADY
                        && authoredPhase != context.phase) return PartLight.NONE;
        boolean[] faceMask = staticPart.faceMask;
        int faceIndex = staticPart.faceIndex;
        if (faceIndex < 0) return PartLight.NONE;

        DetectedLightFace selected = staticPart.face;
        if (selected == null) return PartLight.NONE;
    ...
    private static StaticMetadata staticMetadata(Context context, TextureSet textures) {
        ...
        for (PartReference reference : inventory.parts) {
            ...
            if (part.showModel == false) continue;
            int phase =
                    PlacedLightTexturePhases.phase(
                            ...
                            textures.off,
                            textures.active);
            if (phase == PlacedLightTexturePhases.NONE) continue;
            if (phase == 0) phaseZero++;
            if (phase == 1) phaseOne++;
            boolean[] mask =
    ...
    private static void addPart(
            ...
            String defaultGroup) {
        if (part == null || seen.add(part) == false) return;
    ...
    private static TextureSet loadTextures(TextureKey key) {
        try {
            ...
            if (off == null
                    ...
                    || first.getHeight() != off.getHeight()) return TextureSet.FAILED;
            List<int[]> active = new ArrayList<int[]>();
            active.add(pixels(first));
            if (second != null) active.add(pixels(second));
    ...
    private static BufferedImage read(ResourceLocation location) throws Exception {
        InputStream stream =
    ...
    private static void ensurePreferredFaces(Context context, TextureSet textures) {
        if (context.preferredReady) return;
        ...
        for (PreferredCandidate candidate : candidates) {
            ...
            if (Float.isNaN(span) == false
                ...
                if (outward != null) selected = outward.faceIndex;
            }
    ...
    private static DetectedLightFace preferredFace(
        ...
        for (DetectedLightFace candidate : surface.faces) {
            if (candidate.faceIndex >= mask.length || mask[candidate.faceIndex] == false) continue;
        ...
        while (remaining.isEmpty() == false) {
            ...
            while (queue.isEmpty() == false) {
                ...
                for (Candidate other : remaining) {
                    if (other.phase != current.phase) continue;
    ...
    private static DetectedLightFace face(int index, DetectedLightSurface surface) {
        for (DetectedLightFace face : surface.faces) if (face.faceIndex == index) return face;
        return null;
    }
    ...
    private static DetectedLightFace opposed(
        ...
        for (DetectedLightFace candidate : surface.faces) {
            if (candidate.faceIndex == forward.faceIndex
                    ...
                    || mask[candidate.faceIndex] == false) continue;
            float score =
    ...
    public static final class PartLight {
        static final PartLight NONE =
                new PartLight(false, 0.0F, 0.0F, false, null, 0xFFFFFF);
        ...
        public void drawEmissiveFaces(List<TexturedPolygon> faces, float scale) {
            ...
            try {
                ...
                for (int index = 0; index < count; index++) {
                    if (faceMask[index] == false) continue;
    ...
    private static final class Candidate {
        ...
        Candidate(
            ...
            float[] basis =
                    ClientRollingStockLighting.basis(
    ...
    private static final class MetadataKey {
        ...
        public boolean equals(Object other) {
            if ((other instanceof MetadataKey) == false) return false;
    ...
    private static final class TextureKey {
        ...
        public boolean equals(Object other) {
            ...
            if ((other instanceof TextureKey) == false) return false;
